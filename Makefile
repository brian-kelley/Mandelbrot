...
SMALLFLAGS=-std=c11 -Os
DEBUGFLAGS=-std=c11 -g
FASTCFLAGS=-std=c11 -O0
SIMDFLAGS=-mavx -mavx2
SOURCES=mandelbrot.c bigint.c fixedpoint.c image.c boundary.c lodepng.c

all: BuildDir OutputDir
	nasm -f macho64 routines.asm -o routines.o
	gcc ${CFLAGS} ${SIMDFLAGS} ${INCLUDE} ${SOURCES} routines.o -lc -lpthread -o build/Mandelbrot
...
kernelAsm:
	gcc ${SMALLFLAGS} -masm=intel ${SIMDFLAGS} ${INCLUDE} kernels.c -S -o build/Mandelbrot.asm
